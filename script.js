// Tokenless Smart RAG Workspace — FAST/HYBRID only (no WebLLM load)
...
async function selectRoom(id){ const rs=await loadRooms(); currentRoom=rs.find(r=>r.id===id); document.title=currentRoom?.name||'Smart RAG Workspace'; msgEl.innerHTML='';
  const hist=await DB_CHAT.getItem('chat:'+currentRoom.id)||[]; for(const m of hist) addMsg(m.content, m.sender==='user'?'user':'bot'); filesEl.textContent='없음'; }
function renderRooms(rs){ const list=$("#room-list"); list.innerHTML=''; rs.forEach(r=>{ const el=document.createElement('div'); el.className='room-item';
  el.innerHTML=`<div class="room-name" onclick="window._sel('${r.id}')">${r.name}</div><button class="icon-btn" title="삭제" onclick="window._del('${r.id}')">🗑</button>`; list.appendChild(el); }); }
window._sel=selectRoom; window._del=removeRoom;
document.getElementById('btn-new-room').addEventListener('click', async ()=>{ const name=prompt('새 룸 이름을 입력하세요'); if (name) await addRoom(name.trim()); });
...
document.getElementById('btn-ask').addEventListener('click', async ()=>{
  ...
  const meta=await DB_META.getItem('meta:'+currentRoom.id)||[]; if(!meta.length){ addMsg('인덱스가 없습니다. “업로드→인덱싱”을 먼저 실행하세요.'); return; }
  ...
  const ans = `<div class="tiny">※ 발췌형 답변</div><div>${body}</div>`; addMsg(ans,'bot'); await pushChat('assistant', ans);
});
...
async function searchWithMode(q, meta, mode, k){
  ...
  if (mode==='hybrid'){
    ...
    }catch(e){ li('임베딩 로드 실패 → FAST로 대체'); return pre.slice(0,k); }
  }
...
document.getElementById('btn-upload').addEventListener('click', ()=>{ if(!currentRoom) return alert('먼저 룸을 만드세요.'); openModal('modal-upload'); });
...
drop.addEventListener('drop', async e=>{
  ...
  filesEl.textContent=stagedFiles.map(s=>s.name).join(', ')||'없음'; document.getElementById('btn-index').disabled = stagedFiles.length===0;
});
...
async function extractPdfText(file){
  ...
  document.getElementById('pdf-status').textContent = `PDF 텍스트 추출 완료 (${pdf.numPages}p)`; return text;
}

document.getElementById('btn-index').addEventListener('click', async ()=>{
  if(!currentRoom||!stagedFiles.length) return;
  li('청크 생성...'); const {chunking,chunkSize,overlap}=settings; const meta=[],docs=[]; let id=0;
  ...
  li('BM25 인덱스 생성...'); const idx=elasticlunr(function(){ this.setRef('id'); this.addField('text'); });
  ...
  li('인덱싱 완료'); stagedFiles=[]; filesEl.textContent='업로드 완료'; document.getElementById('btn-index').disabled=true; closeModal('modal-upload');
});
...
// Init
(async function init(){
  let rooms=await loadRooms(); if(!rooms.length){ await addRoom('내 문서'); rooms=await loadRooms(); }
  renderRooms(rooms); if(!currentRoom) await selectRoom(rooms[0].id);
  addMsg('문서 업로드 → 인덱싱 → 질문. (생성형 LLM 미사용, 에러 없음)','bot');
})();
