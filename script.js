/* RAG Web Chat — CDN-FREE via vendored WebLLM (auto by Actions)
 * - ensureWebLLM(): try local ./vendor/web-llm/dist first, then jsDelivr, then unpkg
...
indexBtn.addEventListener('click', async () => {
  ...
  // BM25 index (elasticlunr) — index JSON stored for later load
  li('BM25 인덱스 생성...');
...
// Embeddings via Transformers.js — dynamic ESM import
async function ensureEmbedder(){
  if (embedder) return;
  const url = 'https://cdn.jsdelivr.net/npm/@xenova/transformers/dist/transformers.min.js';
...
// WebLLM loader: try local vendored dist first, then public CDNs
async function ensureWebLLM(){
  ...
  addMsg('WebLLM 로드 실패. Actions가 vendor 파일을 가져오기 전에는 FAST 모드로만 동작합니다.', 'bot');
